...
const { parse: parseIngredient } = require('./ingredientParser');
...
async function parseRecipe(url, useMetric = true) {
  try {
    ...
    const htmlRecipe = await parseHtmlRecipe($, useMetric);
    
    // Validate parsed recipe
    if (!htmlRecipe.ingredients || htmlRecipe.ingredients.length === 0) {
      throw new Error('No ingredients found in recipe');
    }
    if (!htmlRecipe.instructions || htmlRecipe.instructions.length === 0) {
    ...
    if (error.code === 'ECONNABORTED') {
      throw new Error('Request timed out. The recipe website took too long to respond.');
    }
    if (error.response) {
      if (error.response.status === 404) {
        throw new Error('Recipe page not found. Please check the URL.');
      }
      throw new Error(`Failed to fetch recipe: ${error.response.status} ${error.response.statusText}`);
    }
    ...
    for (let i = 0; i < scripts.length; i++) {
      ...
      if (data['@type'] === 'Recipe' || (Array.isArray(data) && data[0]?.['@type'] === 'Recipe')) {
        ...
        const parsedIngredients = await Promise.all(
          (recipe.recipeIngredient || []).map(ing => parseIngredient(ing))
        );
        return {
          title: recipe.name,
 ...
 */
async function parseHtmlRecipe($, useMetric) {
  ...
  for (const el of ingredientElements.toArray()) {
    ...
    if (text) {
      ingredients.push(await parseIngredient(text, useMetric));
    }
  }
